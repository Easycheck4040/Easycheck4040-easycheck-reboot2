...
const countries = [
  ...
  "France", "Deutschland", "United Kingdom", "España", 
  "United States", "Italia", "Belgique", "Suisse", "Luxembourg"
];
...
const defaultRates: Record<string, number> = { 
  'EUR': 1, 'USD': 1.05, 'BRL': 6.15, 'AOA': 930, 
  'MZN': 69, 'CVE': 110.27, 'CHF': 0.94, 'GBP': 0.83 
};
...
const currencySymbols: Record<string, string> = { 
  'EUR': '€', 'USD': '$', 'BRL': 'R$', 'AOA': 'Kz', 
  'MZN': 'MT', 'CVE': 'Esc', 'CHF': 'CHF', 'GBP': '£' 
};
...
export default function Dashboard() {
  ...
  const calculateAmortizationSchedule = (asset: any) => {
      ...
      const rate = asset.amortization_method === 'linear' ? (1 / lifespan) : 0.40;
      ...
      for (let i = 0; i < lifespan; i++) {
          ...
          if (asset.amortization_method === 'linear') {
              ...
              annuity = currentValue * rate;
              if (currentValue / (lifespan - i) > annuity) {
                  annuity = currentValue / (lifespan - i);
              }
          }
          if (currentValue - annuity < 0) annuity = currentValue;
  ...
  useEffect(() => {
      ...
      const updatedItems = invoiceData.items.map(item => ({ ...item, tax: item.tax === 0 && newTax !== 0 ? newTax : item.tax }));
  ...
  const handleSaveInvoice = async () => {
      ...
      if (invoiceData.id) {
          // UPDATE (apagar antigo e criar novo para simplificar itens)
      ...
      setInvoiceData({ id: '', client_id: '', type: 'Fatura', date: new Date().toISOString().split('T')[0], due_date: new Date(new Date().setDate(new Date().getDate() + 30)).toISOString().split('T')[0], exemption_reason: '', items: [{ description: '', quantity: 1, price: 0, tax: 0 }] });
  };
  ...
  const handleDeleteInvoice = async (id: string) => { 
      if (window.confirm("ATENÇÃO: Apagar uma fatura emitida. Continuar?")) { 
  ...
  const generatePDF = () => {
      ...
      if (companyForm.logo_url) { try { doc.addImage(companyForm.logo_url, 'JPEG', 15, 10, 35, 25); } catch { doc.setFontSize(14); doc.text("LOGO", 15, 25); } }
      doc.setFontSize(14); doc.setTextColor(40); doc.text(companyForm.name || 'Minha Empresa', 200, 15, { align: 'right' });
      doc.setFontSize(9); doc.setTextColor(100); doc.text(companyForm.address || '', 200, 22, { align: 'right' }); doc.text(`${companyForm.country}, NIF: ${companyForm.nif || 'N/A'}`, 200, 27, { align: 'right' });
      ...
      doc.setFontSize(10); doc.setTextColor(0); doc.text("Faturar a:", 15, 80); 
      ...
      doc.text(`Data: ${invoiceData.date}`, 140, 86); doc.text(`Vencimento: ${invoiceData.due_date}`, 140, 91);
      ...
      autoTable(doc, { head: [["Descrição", "Qtd", "Preço", "IVA", "Total"]], body: tableRows, startY: 110, theme: 'grid', headStyles: { fillColor: [40, 40, 40] } });
      ...
      if (invoiceData.exemption_reason) { doc.setFontSize(9); doc.setFont("helvetica", "normal"); doc.text(`Isenção: ${invoiceData.exemption_reason}`, 15, finalY + 25); }
      const pageHeight = doc.internal.pageSize.height;
      doc.setFontSize(8); doc.setTextColor(150); if (companyForm.footer) doc.text(companyForm.footer, 105, pageHeight - 15, { align: 'center' });
      doc.text("Processado por EasyCheck", 105, pageHeight - 10, { align: 'center' });
      return doc;
  };
  const handleDownloadPDF = () => { const doc = generatePDF(); doc.save(`Fatura_${Date.now()}.pdf`); handleSaveInvoice(); };
  ...
  const handleCreateAsset = async () => {
    ...
    let error, data;
    if (editingAssetId) {
        const res = await supabase.from('accounting_assets').update({ ...newAsset, purchase_value: valueInEur }).eq('id', editingAssetId).select(); error = res.error; data = res.data;
        if(!error && data) setAssets(prev => prev.map(a => a.id === editingAssetId ? data[0] : a));
    } else {
        const res = await supabase.from('accounting_assets').insert([{ user_id: userData.id, name: newAsset.name, purchase_date: newAsset.purchase_date, purchase_value: valueInEur, lifespan_years: newAsset.lifespan_years, amortization_method: newAsset.amortization_method }]).select(); error = res.error; data = res.data;
        if(!error && data) setAssets([...assets, data[0]]);
    }
    if (!error) { setShowAssetModal(false); setEditingAssetId(null); setNewAsset({ name: '', purchase_date: new Date().toISOString().split('T')[0], purchase_value: '', lifespan_years: 3, amortization_method: 'linear' }); }
  };
  ...
  const handleDeleteAsset = async (id: string) => { if (!window.confirm("Apagar?")) return; const { error } = await supabase.from('accounting_assets').delete().eq('id', id); if (!error) setAssets(prev => prev.filter(a => a.id !== id)); };
  ...
  const handleCreateEntity = async () => {
      ...
      if (editingEntityId) {
          const res = await supabase.from(table).update({ ...newEntity, updated_at: new Date() }).eq('id', editingEntityId).select(); error = res.error; data = res.data;
          if (!error && data) { if (entityType === 'client') setClients(prev => prev.map(c => c.id === editingEntityId ? data[0] : c)); else setSuppliers(prev => prev.map(s => s.id === editingEntityId ? data[0] : s)); }
      } else {
          const res = await supabase.from(table).insert([{ user_id: userData.id, ...newEntity }]).select(); error = res.error; data = res.data;
          if (!error && data) { if (entityType === 'client') setClients([data[0], ...clients]); else setSuppliers([data[0], ...suppliers]); }
      }
  ...
  const handleSaveEntity = handleCreateEntity;
  ...
  const saveDoubtfulDebt = async () => {
      ...
      else { const clientInvoices = realInvoices.filter(inv => inv.client_id === selectedClientForDebt.id && selectedDebtInvoices.includes(inv.id)); amount = clientInvoices.reduce((sum, inv) => sum + inv.total, 0); }
      ...
      if (!error) { setClients(prev => prev.map(c => c.id === selectedClientForDebt.id ? { ...c, ...updates } : c)); setShowDoubtfulModal(false); setManualDebtAmount(''); setSelectedDebtInvoices([]); }
  };
  const toggleClientStatus = handleOpenDoubtful; // Atalho
  ...
  const handleCreateProvision = async () => {
      ...
      if (editingProvisionId) {
           const res = await supabase.from('accounting_provisions').update({ ...newProvision, amount: amountInEur }).eq('id', editingProvisionId).select(); error = res.error; data = res.data;
           if (!error && data) setProvisions(prev => prev.map(p => p.id === editingProvisionId ? data[0] : p));
      } else {
           const res = await supabase.from('accounting_provisions').insert([{ user_id: userData.id, ...newProvision, amount: amountInEur }]).select(); error = res.error; data = res.data;
           if (!error && data) setProvisions([data[0], ...provisions]);
      }
      if (!error) { setShowProvisionModal(false); setEditingProvisionId(null); setNewProvision({ description: '', amount: '', type: 'Riscos e Encargos', date: new Date().toISOString().split('T')[0] }); }
  };
  ...
  return (
    <div className="flex h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-300 font-sans text-gray-900 dark:text-gray-100">
            ...
            <Route path="accounting" element={
                <div className="h-full flex flex-col">
                    ...
                    <div className="flex-1">
                        ...
                        {/* ✅ CLIENTES COM STATUS DUVIDOSO & DÍVIDA */}
                                    ...
                                    <>
                                        ...
                                        <div className="bg-white dark:bg-gray-800 rounded-2xl border dark:border-gray-700 shadow-sm overflow-hidden">
                                            <table className="w-full text-sm text-left"><thead className="bg-gray-50 dark:bg-gray-700 text-gray-500 uppercase text-xs"><tr><th className="px-6 py-3">Número</th><th className="px-6 py-3">Cliente</th><th className="px-6 py-3">Tipo</th><th className="px-6 py-3">Data</th><th className="px-6 py-3 text-right">Total</th><th className="px-6 py-3 text-right">Ação</th></tr></thead>
                                                <tbody>{realInvoices.length === 0 ? <tr><td colSpan={6} className="text-center py-8 text-gray-400">Nenhuma fatura emitida.</td></tr> : realInvoices.map(inv => (<tr key={inv.id} className="border-b dark:border-gray-700"><td className="px-6 py-4 font-mono font-bold text-blue-600">{inv.invoice_number}</td><td className="px-6 py-4">{inv.clients?.name || 'Cliente Removido'}</td><td className="px-6 py-4 text-xs uppercase font-bold">{inv.type}</td><td className="px-6 py-4">{new Date(inv.date).toLocaleDateString()}</td><td className="px-6 py-4 text-right font-bold">{inv.currency} {inv.total}</td><td className="px-6 py-4 text-right flex justify-end gap-2"><button onClick={() => handleEditInvoice(inv)} className="text-blue-500 hover:text-blue-700"><Edit2 size={16}/></button><button onClick={() => handleDeleteInvoice(inv.id)} className="text-red-400 hover:text-red-600"><Trash2 size={16}/></button></td></tr>))}</tbody></table>
                                        </div>
                                    </>
                                ) : (
                        ...
                        {accountingTab === 'chart' && (
                            <div className="bg-white dark:bg-gray-800 rounded-2xl border dark:border-gray-700 shadow-sm overflow-hidden">
                                ...
                                <table className="w-full text-sm text-left"><thead className="bg-gray-50 dark:bg-gray-700 text-gray-500 uppercase text-xs"><tr><th className="px-6 py-3">Código</th><th className="px-6 py-3">Nome</th><th className="px-6 py-3">Tipo</th></tr></thead><tbody>{chartOfAccounts.length === 0 ? <tr><td colSpan={3} className="text-center py-8 text-gray-400">Sem contas.</td></tr> : chartOfAccounts.map(acc => (<tr key={acc.id} className="border-b dark:border-gray-700"><td className="px-6 py-4 font-mono font-bold text-blue-600">{acc.code}</td><td className="px-6 py-4 font-medium">{acc.name}</td><td className="px-6 py-4 uppercase text-xs">{acc.type}</td></tr>))}</tbody></table>
                            </div>
                        )}
                    </div>
                </div>
            ...
            <Route path="company" element={isOwner ? (
                <div className="max-w-4xl mx-auto space-y-6">
                  ...
                  <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border dark:border-gray-700 p-8">
                      ...
                      <div className="mt-6 pt-6 border-t dark:border-gray-700"><h3 className="text-lg font-bold mb-4">Personalização de Faturas</h3><div className="grid grid-cols-1 md:grid-cols-2 gap-6"><div><label className="block text-sm font-bold mb-2">URL do Logótipo</label><div className="flex gap-2"><input value={companyForm.logo_url} onChange={e => setCompanyForm({...companyForm, logo_url: e.target.value})} className="w-full p-3 border rounded-xl dark:bg-gray-900" placeholder="https://..."/><label className="cursor-pointer bg-gray-100 p-3 rounded-xl hover:bg-gray-200"><UploadCloud/><input type="file" className="hidden" onChange={handleLogoUpload}/></label></div></div><div><label className="block text-sm font-bold mb-2">Rodapé da Fatura</label><input value={companyForm.footer} onChange={e => setCompanyForm({...companyForm, footer: e.target.value})} className="w-full p-3 border rounded-xl dark:bg-gray-900" placeholder="Ex: Capital Social 5000€ - CRC Lisboa"/></div></div></div>
                  </div>
                </div>
      ...
      {showDoubtfulModal && selectedClientForDebt && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 w-full max-w-lg shadow-xl border dark:border-gray-700">
                <div className="flex justify-between items-center mb-4"><h3 className="text-xl font-bold flex gap-2 items-center text-red-600"><AlertTriangle size={20}/> Gerir Dívida - {selectedClientForDebt.name}</h3></div>
                <div className="space-y-4">
                    <p className="text-sm text-gray-500">Este cliente será marcado como duvidoso. Indique o valor em dívida.</p>
                    <div className="flex gap-4 mb-4">
                        <button onClick={() => setDebtMethod('manual')} className={`flex-1 py-2 rounded-lg font-bold text-sm ${debtMethod === 'manual' ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-500'}`}>Valor Manual</button>
                        <button onClick={() => setDebtMethod('invoices')} className={`flex-1 py-2 rounded-lg font-bold text-sm ${debtMethod === 'invoices' ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-500'}`}>Selecionar Faturas</button>
                    </div>
                    {debtMethod === 'manual' ? (
                        ...
                        <div className="max-h-40 overflow-y-auto border rounded-xl p-2">
                            {realInvoices.filter(i => i.client_id === selectedClientForDebt.id).map(inv => (
                                <div key={inv.id} className="flex items-center gap-2 p-2 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-lg">
                                    <input type="checkbox" checked={selectedDebtInvoices.includes(inv.id)} onChange={(e) => { if(e.target.checked) setSelectedDebtInvoices([...selectedDebtInvoices, inv.id]); else setSelectedDebtInvoices(selectedDebtInvoices.filter(id => id !== inv.id)); }} />
                                    <span className="text-sm flex-1">{inv.invoice_number}</span>
                                    <span className="font-bold text-sm">{displaySymbol} {inv.total}</span>
                                </div>
                            ))}
                            {realInvoices.filter(i => i.client_id === selectedClientForDebt.id).length === 0 && <p className="text-xs text-gray-400 text-center py-2">Sem faturas registadas.</p>}
                        </div>
                    )}
                </div>
                <div className="flex justify-end gap-3 mt-6"><button onClick={() => setShowDoubtfulModal(false)} className="px-4 py-2 text-gray-500">Cancelar</button><button onClick={saveDoubtfulDebt} className="px-4 py-2 bg-red-600 text-white rounded-lg font-bold">Confirmar Dívida</button></div>
            </div>
        </div>
      )}

      {/* ✅ MODAL PLANO AMORTIZAÇÃO */}
      {showAmortSchedule && selectedAssetForSchedule && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-white dark:bg-gray-800 rounded-2xl p-8 w-full max-w-2xl shadow-xl border dark:border-gray-700 max-h-[80vh] overflow-y-auto">
                <div className="flex justify-between items-center mb-6">
                    <div>
                        <h3 className="text-xl font-bold flex gap-2 items-center"><TrendingUpIcon className="text-blue-500"/> Plano de Amortização</h3>
                        <p className="text-sm text-gray-500">{selectedAssetForSchedule.name} - Método {selectedAssetForSchedule.amortization_method === 'linear' ? 'Linear' : 'Degressivo'}</p>
                    </div>
                    <button onClick={() => setShowAmortSchedule(false)}><X className="text-gray-400"/></button>
                </div>
                <table className="w-full text-sm text-left">
                    <thead className="bg-gray-100 dark:bg-gray-700 text-gray-600 uppercase text-xs"><tr><th className="px-4 py-2">Ano</th><th className="px-4 py-2 text-right">Valor Inicial</th><th className="px-4 py-2 text-right">Anuidade</th><th className="px-4 py-2 text-right">Valor Final</th></tr></thead>
                    <tbody>
                        {calculateAmortizationSchedule(selectedAssetForSchedule).map((row: any) => (
                            <tr key={row.year} className="border-b dark:border-gray-700">
                                ...
                                <td className="px-4 py-3 text-right font-bold">{displaySymbol} {row.endValue.toFixed(2)}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
      ...
      {showAssetModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 w-full max-w-md shadow-xl border dark:border-gray-700">
                <div className="flex justify-between items-center mb-4"><h3 className="text-xl font-bold flex gap-2 items-center text-gray-700 dark:text-white"><Box size={20} className="text-blue-500"/> {editingAssetId ? 'Editar' : 'Novo'} Ativo Imobilizado</h3></div>
                <div className="space-y-4">
