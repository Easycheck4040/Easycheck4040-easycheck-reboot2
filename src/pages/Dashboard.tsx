...
export default function Dashboard() {
  ...
  const [provisions, setProvisions] = useState<any[]>([]); // ✅ NOVO: PROVISÕES
  ...
  const [showProvisionModal, setShowProvisionModal] = useState(false); // ✅ MODAL PROVISÃO
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      if (user) {
        ...
        const { data: prov } = await supabase.from('accounting_provisions').select('*'); if (prov) setProvisions(prov);
      }
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      const fileName = `${userData.id}/logo.${fileExt}`;
  ...
  const generatePDF = () => {
      ...
      doc.setFillColor(245, 247, 250); doc.rect(0, 0, 210, 45, 'F');
      if (companyForm.logo_url) { try { doc.addImage(companyForm.logo_url, 'JPEG', 15, 10, 35, 25); } catch { doc.setFontSize(14); doc.text(companyForm.name.substring(0, 10), 15, 25); } }
      doc.setFontSize(14); doc.setTextColor(40); doc.setFont("helvetica", "bold"); doc.text(companyForm.name || 'Minha Empresa', 200, 15, { align: 'right' });
      doc.setFontSize(9); doc.setFont("helvetica", "normal"); doc.setTextColor(100);
      let companyY = 22; doc.text(companyForm.address || '', 200, companyY, { align: 'right' }); doc.text(`${companyForm.country}, NIF: ${companyForm.nif || 'N/A'}`, 200, companyY + 5, { align: 'right' });
      doc.setFontSize(22); doc.setTextColor(0); doc.text(invoiceData.type.toUpperCase(), 15, 60);
      const prefix = invoiceTypesMap[invoiceData.type] || 'DOC';
      doc.setFontSize(11); doc.setTextColor(100); doc.text(`${prefix} ${new Date().getFullYear()}/${realInvoices.length + 1}`, 15, 66);
      doc.setFontSize(10); doc.setTextColor(0); doc.setFont("helvetica", "bold"); doc.text("Exmo.(s) Sr.(s)", 15, 80);
      let clientY = 86; doc.setFont("helvetica", "bold"); doc.text(client.name, 15, clientY); clientY += 5;
      doc.setFont("helvetica", "normal"); if (client.address) { doc.text(client.address, 15, clientY); clientY += 5; }
      const cityLine = [client.postal_code, client.city, client.country].filter(Boolean).join(' - '); if (cityLine) { doc.text(cityLine, 15, clientY); clientY += 5; }
      ...
      const tableRows = invoiceData.items.map(item => [ item.description, item.quantity, `${displaySymbol} ${item.price.toFixed(2)}`, `${item.tax}%`, `${displaySymbol} ${(item.quantity * item.price).toFixed(2)}` ]);
      autoTable(doc, { head: [["Descrição", "Qtd", "Preço", "IVA", "Total"]], body: tableRows, startY: 110, theme: 'grid', headStyles: { fillColor: [40, 40, 40], textColor: 255 }, styles: { fontSize: 10, cellPadding: 3 } });
      const finalY = (doc as any).lastAutoTable.finalY + 10;
      doc.setFontSize(10); doc.text(`Subtotal:`, 150, finalY); doc.text(`${displaySymbol} ${totals.subtotal.toFixed(2)}`, 195, finalY, { align: 'right' });
      doc.text(`IVA / Taxas:`, 150, finalY + 6); doc.text(`${displaySymbol} ${totals.taxTotal.toFixed(2)}`, 195, finalY + 6, { align: 'right' });
      ...
      if (invoiceData.exemption_reason) { doc.setFontSize(9); doc.setFont("helvetica", "normal"); doc.text(`Motivo de Isenção: ${invoiceData.exemption_reason}`, 15, finalY + 25); }
      const pageHeight = doc.internal.pageSize.height;
      doc.setFontSize(8); doc.setTextColor(150); if (companyForm.footer) doc.text(companyForm.footer, 105, pageHeight - 15, { align: 'center' }); doc.text("Processado por EasyCheck - Software Certificado", 105, pageHeight - 10, { align: 'center' });
      return doc;
  };

  const handleDownloadPDF = () => { const doc = generatePDF(); doc.save(`${invoiceData.type}_${Date.now()}.pdf`); handleSaveInvoice(); };
  const handlePreview = () => { if (!invoiceData.client_id) return alert("Selecione um cliente."); if (invoiceData.items.length === 0) return alert("Adicione itens."); setShowPreviewModal(true); };
  ...
  const handleEditEntity = (entity: any, type: 'client' | 'supplier') => {
      ...
      setEntityType(type); setEditingEntityId(entity.id); setShowEntityModal(true);
  };
  ...
  const handleSaveEntity = async () => {
      ...
      let error = null;
      if (editingEntityId) {
          const res = await supabase.from(table).update({ ...newEntity, updated_at: new Date() }).eq('id', editingEntityId).select(); error = res.error;
          if (!error && res.data) { if (entityType === 'client') setClients(prev => prev.map(c => c.id === editingEntityId ? res.data[0] : c)); else setSuppliers(prev => prev.map(s => s.id === editingEntityId ? res.data[0] : s)); }
      } else {
          const res = await supabase.from(table).insert([{ user_id: userData.id, ...newEntity }]).select(); error = res.error;
          if (!error && res.data) { if (entityType === 'client') setClients([res.data[0], ...clients]); else setSuppliers([res.data[0], ...suppliers]); }
      }
      if (!error) { setShowEntityModal(false); setEditingEntityId(null); setNewEntity({ name: '', nif: '', email: '', address: '', city: '', postal_code: '', country: 'Portugal' }); } else { alert("Erro: " + error.message); }
  };

  // ✅ CLIENTE DUVIDOSO (TOGGLE STATUS)
  const toggleClientStatus = async (client: any) => {
      const newStatus = client.status === 'doubtful' ? 'active' : 'doubtful';
  ...
  const handleCreateProvision = async () => {
      ...
      if (!error && data) { setProvisions([data[0], ...provisions]); setShowProvisionModal(false); setNewProvision({ description: '', amount: '', type: 'Riscos e Encargos', date: new Date().toISOString().split('T')[0] }); }
  };

  // ✅ ATIVOS: APAGAR E CRIAR
  ...
  const handleCreateAsset = async () => {
    ...
    const { data } = await supabase.from('accounting_assets').insert([{ user_id: userData.id, ...newAsset, purchase_value: valueInEur }]).select();
    if (data) { setAssets([...assets, data[0]]); setShowAssetModal(false); setNewAsset({ name: '', purchase_date: new Date().toISOString().split('T')[0], purchase_value: '', lifespan_years: 3, amortization_method: 'linear' }); }
  };
  ...
  const calculateInvoiceTotals = () => { let subtotal = 0; let taxTotal = 0; invoiceData.items.forEach(item => { const lineTotal = item.quantity * item.price; subtotal += lineTotal; taxTotal += lineTotal * (item.tax / 100); }); return { subtotal, taxTotal, total: subtotal + taxTotal }; };

  const handleSaveInvoice = async () => {
    const totals = calculateInvoiceTotals();
    const prefix = invoiceTypesMap[invoiceData.type] || 'DOC';
    const docNumber = `${prefix} ${new Date().getFullYear()}/${realInvoices.length + 1}`;
    const { data: invoice, error } = await supabase.from('invoices').insert([{ user_id: userData.id, client_id: invoiceData.client_id, type: invoiceData.type, invoice_number: docNumber, date: invoiceData.date, due_date: invoiceData.due_date, exemption_reason: invoiceData.exemption_reason, subtotal: totals.subtotal, tax_total: totals.taxTotal, total: totals.total, currency: currentCurrency, status: 'sent' }]).select().single();
    if (error) return alert("Erro: " + error.message);
    const itemsToInsert = invoiceData.items.map(item => ({ invoice_id: invoice.id, description: item.description, quantity: item.quantity, unit_price: item.price, tax_rate: item.tax }));
    await supabase.from('invoice_items').insert(itemsToInsert);
    const { data: updatedInvoices } = await supabase.from('invoices').select('*, clients(name)').order('created_at', { ascending: false }); if (updatedInvoices) setRealInvoices(updatedInvoices);
    setShowPreviewModal(false); setShowInvoiceForm(false);
    setInvoiceData({ client_id: '', type: 'Fatura', date: new Date().toISOString().split('T')[0], due_date: new Date(new Date().setDate(new Date().getDate() + 30)).toISOString().split('T')[0], exemption_reason: '', items: [{ description: '', quantity: 1, price: 0, tax: 0 }] });
  };
  
  const handleDeleteInvoice = async (id: string) => { if (window.confirm("ATENÇÃO: Apagar fatura emitida. Continuar?")) { if (window.prompt("Escreva 'APAGAR' para confirmar:") === 'APAGAR') { const { error } = await supabase.from('invoices').delete().eq('id', id); if (!error) setRealInvoices(prev => prev.filter(i => i.id !== id)); } } };
  ...
  const handleCreateEntity = handleSaveEntity; 
  const handleDeleteEntity = async (id: string, type: 'client' | 'supplier') => { if (!window.confirm("Apagar?")) return; const table = type === 'client' ? 'clients' : 'suppliers'; const { error } = await supabase.from(table).delete().eq('id', id); if (!error) { if (type === 'client') setClients(prev => prev.filter(c => c.id !== id)); else setSuppliers(prev => prev.filter(s => s.id !== id)); } };
  const handleDeleteTransaction = async (id: string) => { if (!window.confirm("Eliminar?")) return; const { error } = await supabase.from('transactions').delete().eq('id', id); if (!error) setTransactions(prev => prev.filter(t => t.id !== id)); };
  ...
  return (
    <div className="flex h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-300 font-sans text-gray-900 dark:text-gray-100">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-white dark:bg-gray-800 border-r dark:border-gray-700 transform md:translate-x-0 transition-transform ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="h-20 flex items-center px-6 border-b dark:border-gray-700"><Link to="/" className="flex items-center gap-3"><img src="/logopequena.PNG" className="h-8 w-auto"/><span className="font-bold text-xl">EasyCheck</span></Link></div>
        ...
        <div className="absolute bottom-0 w-full p-4 border-t dark:border-gray-700 bg-white dark:bg-gray-800"><button onClick={handleLogout} className="flex items-center gap-3 w-full px-4 py-3 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl transition-colors font-medium"><LogOut className="w-5 h-5" /> {t('nav.logout')}</button></div>
      </aside>
      ...
      <main className="flex-1 md:ml-64 flex flex-col h-screen overflow-hidden relative">
        <header className="h-20 bg-white dark:bg-gray-800 border-b dark:border-gray-700 flex justify-between px-8 shadow-sm z-20 items-center">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-xl font-bold flex items-center gap-2">{profileData?.country && <span className="text-2xl">{profileData.country === 'Portugal' ? '🇵🇹' : profileData.country === 'Brasil' ? '🇧🇷' : ''}</span>}{menuItems.find(i => i.path === location.pathname)?.label || 'Dashboard'}</h2>
          </div>
          <div className="flex items-center gap-4">
            ...
            <div className="relative"><button onClick={() => setIsProfileDropdownOpen(!isProfileDropdownOpen)} className="w-10 h-10 bg-gradient-to-tr from-blue-600 to-purple-600 rounded-full text-white font-bold shadow-md cursor-pointer hover:opacity-90">{getInitials(profileData?.full_name)}</button>
              {isProfileDropdownOpen && (<div className="absolute top-16 right-0 w-72 bg-white dark:bg-gray-800 rounded-xl shadow-xl border dark:border-gray-700 z-40 overflow-hidden"><div className="px-4 py-4 border-b dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50"><p className="font-bold truncate">{profileData?.full_name}</p><p className="text-xs text-gray-500 truncate mb-2">{profileData?.company_name}</p></div><button onClick={() => {setIsProfileModalOpen(true); setIsProfileDropdownOpen(false)}} className="w-full text-left px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-700 flex gap-2 text-sm font-medium"><User className="w-4 h-4"/> {t('profile.edit')}</button></div>)}
            </div>
          </div>
        </header>
        ...
        <div className="flex-1 overflow-y-auto bg-gray-50 dark:bg-gray-900 p-8">
          <Routes>
            ...
            <Route path="accounting" element={
                <div className="h-full flex flex-col">
                    <div className="flex gap-2 border-b dark:border-gray-700 pb-2 mb-6 overflow-x-auto">
                        {[ { id: 'overview', label: 'Geral', icon: PieChart }, { id: 'clients', label: 'Clientes', icon: Briefcase }, { id: 'invoices', label: 'Faturas', icon: FileText }, { id: 'suppliers', label: 'Fornecedores', icon: Truck }, { id: 'assets', label: 'Ativos', icon: Box }, { id: 'provisions', label: 'Provisões', icon: AlertOctagon } ].map(tab => (
                            <button key={tab.id} onClick={() => setAccountingTab(tab.id)} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-colors ${accountingTab === tab.id ? 'bg-white dark:bg-gray-800 text-blue-600 shadow-sm border dark:border-gray-600' : 'text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800'}`}><tab.icon size={16}/> {tab.label}</button>
                        ))}
                    </div>
                    <div className="flex-1">
                        ...
                        {accountingTab === 'clients' && (
                            <div className="bg-white dark:bg-gray-800 rounded-2xl border dark:border-gray-700 shadow-sm overflow-hidden">
                                ...
                                <table className="w-full text-sm text-left"><thead className="bg-gray-50 dark:bg-gray-700 text-gray-500 uppercase text-xs"><tr><th className="px-6 py-3">Nome</th><th className="px-6 py-3">NIF</th><th className="px-6 py-3">Email</th><th className="px-6 py-3">Status</th><th className="px-6 py-3 text-right">Ação</th></tr></thead>
                                    <tbody>{clients.length === 0 ? <tr><td colSpan={5} className="text-center py-8 text-gray-400">Nenhum cliente registado.</td></tr> : clients.map(c => (
                                        <tr key={c.id} className={`border-b dark:border-gray-700 ${c.status === 'doubtful' ? 'bg-red-50 dark:bg-red-900/20' : ''}`}>
                                            ...
                                            <td className="px-6 py-4 text-right flex justify-end gap-2">
                                                <button onClick={() => toggleClientStatus(c)} title="Marcar como Duvidoso" className={`p-1 rounded-full ${c.status === 'doubtful' ? 'text-red-600 bg-red-100' : 'text-gray-400 hover:bg-gray-100'}`}><AlertTriangle size={16}/></button>
                        ...
                        {accountingTab === 'provisions' && (
                            <div className="space-y-6">
                                <div className="flex justify-between items-center"><h3 className="font-bold text-lg">Provisões do Exercício</h3><button onClick={() => setShowProvisionModal(true)} className="bg-blue-600 text-white px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-blue-700 shadow-lg"><Plus size={18}/> Nova Provisão</button></div>
                                <div className="bg-white dark:bg-gray-800 rounded-2xl border dark:border-gray-700 shadow-sm overflow-hidden">
                                    <table className="w-full text-sm text-left"><thead className="bg-gray-50 dark:bg-gray-700 text-gray-500 uppercase text-xs"><tr><th className="px-6 py-3">Data</th><th className="px-6 py-3">Descrição</th><th className="px-6 py-3">Tipo</th><th className="px-6 py-3 text-right">Valor Estimado</th></tr></thead>
                                    <tbody>{provisions.length === 0 ? <tr><td colSpan={4} className="text-center py-8 text-gray-400">Sem provisões registadas.</td></tr> : provisions.map(p => (<tr key={p.id} className="border-b dark:border-gray-700"><td className="px-6 py-4">{new Date(p.date).toLocaleDateString()}</td><td className="px-6 py-4 font-medium">{p.description}</td><td className="px-6 py-4"><span className="bg-yellow-100 text-yellow-800 px-2 py-1 rounded-full text-xs font-bold">{p.type}</span></td><td className="px-6 py-4 text-right font-bold text-yellow-600">{displaySymbol} {(p.amount * conversionRate).toFixed(2)}</td></tr>))}</tbody></table>
                                </div>
                            </div>
                        )}
                        {accountingTab === 'suppliers' && (
                            <div className="bg-white dark:bg-gray-800 rounded-2xl border dark:border-gray-700 shadow-sm overflow-hidden">
                                <div className="p-4 border-b dark:border-gray-700 flex justify-between items-center"><h3 className="font-bold flex items-center gap-2"><Truck size={18}/> Fornecedores</h3><button onClick={() => {setEditingEntityId(null); setNewEntity({ name: '', nif: '', email: '', address: '', city: '', postal_code: '', country: 'Portugal' }); setEntityType('supplier'); setShowEntityModal(true)}} className="bg-blue-600 text-white px-3 py-1.5 rounded-lg text-sm flex gap-2 items-center"><Plus size={16}/> Novo Fornecedor</button></div>
                                <table className="w-full text-sm text-left"><thead className="bg-gray-50 dark:bg-gray-700 text-gray-500 uppercase text-xs"><tr><th className="px-6 py-3">Nome</th><th className="px-6 py-3">NIF</th><th className="px-6 py-3">Email</th><th className="px-6 py-3">Categoria</th><th className="px-6 py-3 text-right">Ação</th></tr></thead>
                                    <tbody>{suppliers.length === 0 ? <tr><td colSpan={5} className="text-center py-8 text-gray-400">Nenhum fornecedor registado.</td></tr> : suppliers.map(s => (<tr key={s.id} className="border-b dark:border-gray-700"><td className="px-6 py-4 font-medium">{s.name}</td><td className="px-6 py-4">{s.nif || '-'}</td><td className="px-6 py-4">{s.email || '-'}</td><td className="px-6 py-4">{s.category || '-'}</td><td className="px-6 py-4 text-right flex justify-end gap-2"><button onClick={() => handleEditEntity(s, 'supplier')} className="text-blue-500 hover:text-blue-700"><Edit2 size={16}/></button><button onClick={() => handleDeleteEntity(s.id, 'supplier')} className="text-red-400 hover:text-red-600"><Trash2 size={16}/></button></td></tr>))}</tbody></table>
                            </div>
                        )}
                        {accountingTab === 'invoices' && (
                            <div className="space-y-6">
                                {!showInvoiceForm ? (
                                    <>
                                        <div className="flex justify-between items-center"><h3 className="font-bold text-lg">Faturas Emitidas</h3><button onClick={() => setShowInvoiceForm(true)} className="bg-blue-600 text-white px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-blue-700 shadow-lg"><Plus size={18}/> Criar Nova Fatura</button></div>
                                        <div className="bg-white dark:bg-gray-800 rounded-2xl border dark:border-gray-700 shadow-sm overflow-hidden">
                                            <table className="w-full text-sm text-left"><thead className="bg-gray-50 dark:bg-gray-700 text-gray-500 uppercase text-xs"><tr><th className="px-6 py-3">Número</th><th className="px-6 py-3">Cliente</th><th className="px-6 py-3">Tipo</th><th className="px-6 py-3">Data</th><th className="px-6 py-3 text-right">Total</th><th className="px-6 py-3 text-right">Ação</th></tr></thead>
                                                <tbody>{realInvoices.length === 0 ? <tr><td colSpan={6} className="text-center py-8 text-gray-400">Nenhuma fatura emitida.</td></tr> : realInvoices.map(inv => (<tr key={inv.id} className="border-b dark:border-gray-700"><td className="px-6 py-4 font-mono font-bold text-blue-600">{inv.invoice_number}</td><td className="px-6 py-4">{inv.clients?.name || 'Cliente Removido'}</td><td className="px-6 py-4 text-xs uppercase font-bold">{inv.type}</td><td className="px-6 py-4">{new Date(inv.date).toLocaleDateString()}</td><td className="px-6 py-4 text-right font-bold">{inv.currency} {inv.total}</td><td className="px-6 py-4 text-right"><button onClick={() => handleDeleteInvoice(inv.id)} className="text-red-400 hover:text-red-600"><Trash2 size={16}/></button></td></tr>))}</tbody></table>
                                        </div>
                                    </>
                                ) : (
                                    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border dark:border-gray-700 p-8 animate-fade-in-up">
                                        <div className="flex justify-between items-start mb-8 pb-6 border-b dark:border-gray-700"><div><h2 className="text-2xl font-bold flex items-center gap-2"><FileText className="text-blue-600"/> Emitir Novo Documento</h2><p className="text-gray-500 text-sm mt-1">Selecione o tipo de documento e preencha os dados.</p></div><button onClick={() => setShowInvoiceForm(false)} className="text-gray-400 hover:text-gray-600"><X size={24}/></button></div>
                                        <div className="mb-6"><label className="block text-sm font-bold mb-2">Tipo de Documento</label><div className="flex gap-2 overflow-x-auto pb-2">{invoiceTypes.map(t => (<button key={t} onClick={() => setInvoiceData({...invoiceData, type: t})} className={`px-4 py-2 rounded-full text-xs font-bold whitespace-nowrap border transition-all ${invoiceData.type === t ? 'bg-blue-600 text-white border-blue-600' : 'bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-600 hover:bg-gray-50'}`}>{t}</button>))}</div></div>
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                                            <div><label className="block text-sm font-bold mb-2">Cliente</label><select className="w-full p-3 border rounded-xl dark:bg-gray-900 outline-none focus:ring-2 focus:ring-blue-500" value={invoiceData.client_id} onChange={e => setInvoiceData({...invoiceData, client_id: e.target.value})}><option value="">Selecione um cliente...</option>{clients.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select>{clients.length === 0 && <p className="text-xs text-red-500 mt-1">Crie um cliente primeiro na aba Clientes.</p>}</div>
                                        ...
                                        {invoiceData.exemption_reason && (<div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700 p-4 rounded-xl mb-6 flex gap-3 items-center"><AlertCircle className="text-yellow-600"/><div><p className="font-bold text-yellow-800 dark:text-yellow-200 text-sm">Regime de Isenção Aplicado</p><p className="text-xs text-yellow-700 dark:text-yellow-300">{invoiceData.exemption_reason}</p></div></div>)}
                                        <div className="mb-8">
                                            <table className="w-full text-sm"><thead><tr className="border-b dark:border-gray-700 text-left text-gray-500 uppercase text-xs"><th className="py-2 w-1/2">Descrição</th><th className="py-2 w-20 text-center">Qtd</th><th className="py-2 w-32 text-right">Preço Un.</th><th className="py-2 w-24 text-right">IVA %</th><th className="py-2 w-32 text-right">Total</th><th className="py-2 w-10"></th></tr></thead>
                                                <tbody className="divide-y dark:divide-gray-700">{invoiceData.items.map((item, index) => (
                                                    <tr key={index}><td className="py-3"><input className="w-full bg-transparent outline-none font-medium" placeholder="Nome do produto/serviço" value={item.description} onChange={e => updateInvoiceItem(index, 'description', e.target.value)}/></td><td className="py-3"><input type="number" className="w-full bg-transparent outline-none text-center" value={item.quantity} onChange={e => updateInvoiceItem(index, 'quantity', e.target.value)}/></td><td className="py-3"><input type="number" className="w-full bg-transparent outline-none text-right" value={item.price} onChange={e => updateInvoiceItem(index, 'price', e.target.value)}/></td>
                                                        <td className="py-3 flex items-center justify-end gap-2 relative"><input type="number" className="w-16 bg-transparent outline-none text-right font-bold border-b border-dashed border-gray-300 focus:border-blue-500" value={item.tax} onChange={e => updateInvoiceItem(index, 'tax', e.target.value)}/><div className="relative group"><button className="p-1 bg-gray-100 dark:bg-gray-700 rounded-full hover:bg-blue-100 dark:hover:bg-blue-900/50 text-gray-500 hover:text-blue-600 transition-colors"><Edit2 size={12}/></button><div className="absolute right-0 top-full mt-2 w-48 bg-white dark:bg-gray-800 border dark:border-gray-700 shadow-xl rounded-xl p-2 z-50 hidden group-hover:block animate-fade-in-up"><p className="text-[10px] uppercase text-gray-400 font-bold mb-2 px-2 border-b dark:border-gray-700 pb-1">Taxas {companyForm.country}</p><div className="grid grid-cols-2 gap-1">{getCurrentCountryVatRates().map(rate => (<button key={rate} onClick={() => updateInvoiceItem(index, 'tax', rate.toString())} className="text-center px-2 py-1.5 text-xs hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg font-medium transition-colors">{rate}%</button>))}</div></div></div></td>
                                                        <td className="py-3 text-right font-bold">{displaySymbol} {(item.quantity * item.price).toFixed(2)}</td><td className="py-3 text-center"><button onClick={() => handleRemoveInvoiceItem(index)} className="text-red-400 hover:text-red-600"><Trash2 size={16}/></button></td></tr>))}
                                                </tbody>
                                            </table>
                                        ...
                                        <div className="flex justify-end border-t dark:border-gray-700 pt-6"><div className="w-64 space-y-2"><div className="flex justify-between text-gray-500"><span>Subtotal</span><span>{displaySymbol} {invoiceTotals.subtotal.toFixed(2)}</span></div><div className="flex justify-between text-gray-500"><span>IVA / Taxas</span><span>{displaySymbol} {invoiceTotals.taxTotal.toFixed(2)}</span></div><div className="flex justify-between text-xl font-bold text-gray-800 dark:text-white pt-2 border-t dark:border-gray-700"><span>Total</span><span>{displaySymbol} {invoiceTotals.total.toFixed(2)}</span></div></div></div>
                                        <div className="flex justify-end gap-4 mt-8"><button onClick={() => setShowInvoiceForm(false)} className="px-6 py-3 rounded-xl border font-medium hover:bg-gray-50 dark:hover:bg-gray-700">Cancelar</button><button onClick={handlePreview} className="px-6 py-3 rounded-xl bg-blue-600 text-white font-bold hover:bg-blue-700 shadow-lg flex items-center gap-2"><Eye size={20}/> Pré-visualizar</button></div>
                                    </div>
                                )}
                            </div>
                        )}
                        {/* ✅ ATIVOS COM AMORTIZAÇÃO E DELETE */}
                        {accountingTab === 'assets' && (
                            <div className="space-y-6"><div className="flex justify-between items-center"><h3 className="font-bold text-lg">Mapa de Amortizações</h3><button onClick={() => setShowAssetModal(true)} className="bg-blue-600 text-white px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-blue-700 shadow-lg"><Plus size={18}/> Novo Ativo</button></div><div className="bg-white dark:bg-gray-800 rounded-2xl border dark:border-gray-700 shadow-sm overflow-hidden"><table className="w-full text-sm text-left"><thead className="bg-gray-50 dark:bg-gray-700 text-gray-500 uppercase text-xs"><tr><th className="px-6 py-3">Ativo</th><th className="px-6 py-3">Data</th><th className="px-6 py-3 text-right">Valor</th><th className="px-6 py-3 text-right">Método</th><th className="px-6 py-3 text-right">Amort. Anual</th><th className="px-6 py-3"></th></tr></thead><tbody>{assets.length === 0 ? <tr><td colSpan={6} className="text-center py-8 text-gray-400">Nenhum ativo registado.</td></tr> : assets.map(a => (<tr key={a.id} className="border-b dark:border-gray-700"><td className="px-6 py-4 font-medium">{a.name}</td><td className="px-6 py-4">{new Date(a.purchase_date).toLocaleDateString()}</td><td className="px-6 py-4 text-right">{displaySymbol} {(a.purchase_value * conversionRate).toFixed(2)}</td><td className="px-6 py-4 text-right capitalize">{a.amortization_method || 'Linear'}</td><td className="px-6 py-4 text-right font-bold text-orange-500">{displaySymbol} {((a.amortization_method === 'degressive' ? (a.purchase_value * 0.4) : (a.purchase_value / a.lifespan_years)) * conversionRate).toFixed(2)}</td><td className="px-6 py-4 text-right"><button onClick={() => handleDeleteAsset(a.id)} className="text-red-400 hover:text-red-600"><Trash2 size={16}/></button></td></tr>))}</tbody></table></div></div>
                        )}
                    </div>
                </div>
            } />
            ...
            <Route path="company" element={isOwner ? (
                <div className="max-w-4xl mx-auto space-y-6">
                    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border dark:border-gray-700 p-8">
                      <div className="flex justify-between items-center mb-6"><h2 className="text-xl font-bold flex items-center gap-2"><Building2 className="text-blue-600"/> Dados Fiscais da Empresa</h2><button onClick={handleSaveCompany} disabled={savingCompany} className="bg-blue-600 text-white px-4 py-2 rounded-xl flex items-center gap-2 font-bold hover:bg-blue-700 shadow-md transition-all">{savingCompany ? 'A guardar...' : <><Save size={18}/> Guardar Dados</>}</button></div>
                      ...
                      <div className="mt-8 pt-8 border-t dark:border-gray-700"><h3 className="text-lg font-bold mb-4">Personalização de Faturas</h3><div className="grid grid-cols-1 md:grid-cols-2 gap-8"><div><label className="block text-sm font-bold mb-2">Logótipo da Empresa</label><div className="flex items-center gap-4">{companyForm.logo_url ? <img src={companyForm.logo_url} className="h-16 w-auto object-contain border rounded-lg p-1"/> : <div className="h-16 w-16 bg-gray-100 rounded-lg flex items-center justify-center text-gray-400"><ImageIcon/></div>}<label className="cursor-pointer bg-blue-50 hover:bg-blue-100 text-blue-600 px-4 py-2 rounded-lg font-bold text-sm transition-colors flex items-center gap-2"><UploadCloud size={16}/> {uploadingLogo ? 'A carregar...' : 'Carregar Imagem'}<input type="file" accept="image/*" className="hidden" onChange={handleLogoUpload} disabled={uploadingLogo}/></label></div></div><div><label className="block text-sm font-bold mb-2">Rodapé da Fatura</label><textarea value={companyForm.footer} onChange={e => setCompanyForm({...companyForm, footer: e.target.value})} className="w-full p-3 border rounded-xl dark:bg-gray-900 h-24 resize-none" placeholder="Ex: Capital Social 5000€ - CRC Lisboa"/></div></div></div>
                  </div>
                </div>
            ...
            <Route path="settings" element={
                 <div className="max-w-4xl mx-auto space-y-6">
                    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border dark:border-gray-700 p-8"><h2 className="text-2xl font-bold mb-6 flex gap-2 items-center"><Settings/> Definições Globais</h2><div className="grid grid-cols-1 md:grid-cols-2 gap-6"><div><label className="block text-sm font-bold mb-1">País da Sede</label><select value={companyForm.country} onChange={handleCountryChange} className="w-full p-3 border rounded-xl dark:bg-gray-900 dark:border-gray-600">{countries.map(c => <option key={c} value={c}>{c}</option>)}</select></div><div><label className="block text-sm font-bold mb-1">Moeda Principal</label><input value={companyForm.currency} onChange={e => setCompanyForm({...companyForm, currency: e.target.value})} className="w-full p-3 border rounded-xl dark:bg-gray-900 dark:border-gray-600" disabled/></div></div></div>
                    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border dark:border-gray-700 p-8"><h2 className="text-xl font-bold mb-4 flex gap-2 items-center text-blue-600"><RefreshCw/> Gestão de Taxas de Câmbio</h2><div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">{Object.keys(defaultRates).filter(k => k !== 'EUR').map(currency => (<div key={currency} className="bg-gray-50 dark:bg-gray-900 p-4 rounded-xl border dark:border-gray-700 flex items-center justify-between"><div><p className="font-bold text-lg">{currency}</p><p className="text-xs text-gray-500">{currencyNames[currency]}</p></div><div className="flex items-center gap-2"><span className="text-gray-400 text-sm">1 € =</span><input type="number" step="0.01" value={exchangeRates[currency]} onChange={(e) => handleRateChange(currency, e.target.value)} className="w-20 p-2 text-right font-bold rounded-lg border focus:ring-2 focus:ring-blue-500 outline-none dark:bg-gray-800 dark:border-gray-600"/></div></div>))}</div></div>
            ...
            <Route path="hr" element={<div className="text-center py-20 text-gray-500"><Users className="w-16 h-16 mx-auto mb-4 opacity-50"/><h3>Gestão de Recursos Humanos</h3></div>} />
            <Route path="communication" element={<div className="text-center py-20 text-gray-500"><Mail className="w-16 h-16 mx-auto mb-4 opacity-50"/><h3>Centro de Comunicação</h3></div>} />
            <Route path="marketing" element={<div className="text-center py-20 text-gray-500"><BarChart3 className="w-16 h-16 mx-auto mb-4 opacity-50"/><h3>Marketing & CRM</h3></div>} />
      ...
      {isProfileModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 w-full max-w-lg shadow-2xl border dark:border-gray-700 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex justify-end gap-3 mt-8 pt-4 border-t dark:border-gray-700"><button onClick={() => setIsProfileModalOpen(false)} className="px-5 py-2.5 border rounded-xl">{t('common.cancel')}</button><button onClick={handleSaveProfile} disabled={savingProfile} className="px-5 py-2.5 bg-blue-600 text-white rounded-xl font-bold">{savingProfile ? 'Guardando...' : t('common.save')}</button></div>
          </div>
        </div>
      )}
      ...
      {showEntityModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 w-full max-w-lg shadow-xl border dark:border-gray-700">
                <div className="flex justify-between items-center mb-4"><h3 className="text-xl font-bold flex gap-2 items-center text-gray-700 dark:text-white">{entityType === 'client' ? <Briefcase size={20} className="text-blue-500"/> : <Truck size={20} className="text-orange-500"/>} {editingEntityId ? 'Editar' : 'Novo'} {entityType === 'client' ? 'Cliente' : 'Fornecedor'}</h3></div>
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4"><div><label className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1 block">Cidade</label><input placeholder="Lisboa" value={newEntity.city} onChange={e => setNewEntity({...newEntity, city: e.target.value})} className="w-full p-3 border dark:border-gray-600 rounded-xl dark:bg-gray-900 bg-gray-50 outline-none"/></div><div><label className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1 block">País</label><select value={newEntity.country} onChange={e => setNewEntity({...newEntity, country: e.target.value})} className="w-full p-3 border dark:border-gray-600 rounded-xl dark:bg-gray-900 bg-gray-50 outline-none">{countries.map(c => <option key={c} value={c}>{c}</option>)}</select></div></div>
                </div>
                <div className="flex justify-end gap-3 mt-8 pt-4 border-t dark:border-gray-700"><button onClick={() => setShowEntityModal(false)} className="px-6 py-3 text-gray-500 font-medium hover:bg-gray-100 dark:hover:bg-gray-700 rounded-xl transition-colors">Cancelar</button><button onClick={handleSaveEntity} className="px-6 py-3 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 shadow-lg transition-transform active:scale-95">{editingEntityId ? 'Atualizar' : 'Criar Ficha'}</button></div>
            </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 w-full max-w-md shadow-xl border dark:border-gray-700">
                ...
                <div className="space-y-4">
                    ...
                    <div><label className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1 block">Categoria</label><input placeholder="Sem categoria" list="categories" value={newTransaction.category} onChange={e => setNewTransaction({...newTransaction, category: e.target.value})} className="w-full p-3 border dark:border-gray-600 rounded-xl dark:bg-gray-900 bg-gray-50 focus:ring-2 focus:ring-blue-500 outline-none"/><datalist id="categories"><option value="Geral"/><option value="Alimentação"/><option value="Transporte"/><option value="Serviços"/><option value="Vendas"/><option value="Salários"/></datalist></div>
                </div>
                <div className="flex justify-end gap-3 mt-8 pt-4 border-t dark:border-gray-700"><button onClick={() => setShowTransactionModal(false)} className="px-6 py-3 text-gray-500 font-medium hover:bg-gray-100 dark:hover:bg-gray-700 rounded-xl transition-colors">Cancelar</button><button onClick={handleCreateTransaction} className="px-6 py-3 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 shadow-lg transition-transform active:scale-95">Registar</button></div>
            </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 w-full max-w-md shadow-xl border dark:border-gray-700">
                ...
                <div className="space-y-4">
                    ...
                    <div><label className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1 block">Vida Útil (Anos)</label><select value={newAsset.lifespan_years} onChange={e => setNewAsset({...newAsset, lifespan_years: parseInt(e.target.value)})} className="w-full p-3 border dark:border-gray-600 rounded-xl dark:bg-gray-900 bg-gray-50 outline-none"><option value="3">3 Anos (Hardware/Software)</option><option value="4">4 Anos (Veículos Ligeiros)</option><option value="5">5 Anos (Mobiliário/Equipamento)</option><option value="8">8 Anos (Maquinaria Pesada)</option><option value="20">20 Anos (Edifícios)</option></select></div>
            ...
            <div className="bg-white dark:bg-gray-800 w-full h-full max-w-5xl rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-scale-in">
                <div className="bg-gray-900 text-white p-4 flex justify-between items-center shrink-0">
                    ...
                    <div className="flex gap-4">
                        ...
                        <button onClick={handleDownloadPDF} className="bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded-lg font-bold flex items-center gap-2"><Download size={18}/> Baixar PDF e Gravar</button>
                    </div>
                </div>
